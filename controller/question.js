...
const addAnswerToQuestion = async (req, res) => {
  try {
    ...
    await question.save();
    res.status(200).json({ msg: "Answer added successfully", question });
  } catch (error) {
    console.error(error);
...
const removeAnswerToQuestion = async (req, res) => {
  try {
    const questionId = req.params.questionId;
    const answerId = req.params.answerId;
    const question = await Ques.findById(questionId);
    ...
    res.status(200).json({ msg: "Answer removed successfully", question });
  } catch (e) {
    console.error(e);
    res.status(500).json({ msg: "An error occurred" });
  }
